...
//	< 0 on failure.
static int
fsipc(u_int type, void *fsreq, u_longlong dstva, u_longlong *perm)
{
	u_longlong whom;
	// NOTEICE: Our file system no.1 process!
	ipc_send(fileSystemPid, type, (u_longlong)fsreq, PTE_V | PTE_R);
...
fsipc_map(u_int fileid, u_int offset, u_int dstva)
{
	...
	if ((perm & ~(PTE_R | PTE_G)) != (PTE_V)) {
        printf("fsipc_map: unexpected permissions %08x for dstva %08x", perm, dstva);
        while(1);
//		uPanic("fsipc_map: unexpected permissions %08x for dstva %08x", perm, dstva);
	}

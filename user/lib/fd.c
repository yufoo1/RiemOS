...
//		}
//
//		if (((* vpt)[va / PAGE_SIZE] & PTE_V) == 0) {	//the fd is not used
//			*fd = (struct Fd *)va;
//			return 0;
//		}
	}
...
//				if ((r = mapMemoryFromTo(0, ova + i, 0, nva + i,
//										 pte & (PTE_V | PTE_R | PTE_G))) < 0) {
//					goto err;
//				}
//			}
//		}
//	}
//
//	if ((r = mapMemoryFromTo(0, (u_int)oldfd, 0, (u_int)newfd,
//							((*vpt)[VPN(oldfd)]) & (PTE_V | PTE_R | PTE_G))) < 0) {
//		goto err;
//	}
	return newfdnum;

err:
	unmapMemoryFrom(0, (u_int)newfd);

	for (i = 0; i < PDMAP; i += PGSIZE) {
		unmapMemoryFrom(0, nva + i);
	}

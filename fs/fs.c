...
u_int
va_is_mapped(u_longlong va)
{
	return checkVaIsMap(va);
}
...
{
	u_longlong va = diskaddr(blockno);
	if (va_is_mapped(va)) {
		return va;
	}
	return 0;
...
map_block(u_int blockno)
{
	...
	return memoryAlloc(0, diskaddr(blockno), PTE_R | PTE_V);
}
